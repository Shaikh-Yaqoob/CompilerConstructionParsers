...
def extract_grammar(string):
    ...
    for i in range(2, len(split)):
        if split[i].isupper():
            ...
            if split[i] == "micro":
                print(f"I said don't use micro as a terminal \n{split}")
                exit(0)
            if i - last_l != 0:
            ...
            print(f"I said don't use -> as a terminal \n{split}")
            exit(0)
        elif split[i] == "$":
            print(f"I said don't use $ as a terminal \n{split}")
            exit(0)
    ...
    while True:
        ...
        if top in terminals:
            ...
            if string_to_parse[pointer] in terminals or string_to_parse[pointer] == "$":
                ...
                if parse_pointer != -1:
                    stack.exclear
                    cltend(RHS[parse_pointer])
                    parse_stack[1] = [' '.join(stack)]
                    current_rule[1] = [' '.join(rules[parse_pointer])]
                else:
                    Error = True
                    current_rule[1] = [f" {top} to terminal {string_to_parse[pointer]} (string not accepted)"]
            else:
                Error = True
